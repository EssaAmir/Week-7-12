...
def hash_password(plain_text_password):
    ...
    # gensalt() generates a new salt, which is included in the resulting hash.
    salt = bcrypt.gensalt()
    hashed = bcrypt.hashpw(password_bytes, salt)
    # The result includes the hash, the salt, and the cost factor.
...
def _save_users(users):
    ...
    with open(USER_FILE, 'w') as f:
        for username, data in users.items():
            # Write back in the format: username,password_hash,role
...
def login_user(username, password):
    ...
    try:
        ...
        # User data structure is {'password_hash': hash, 'role': role}
        ...
        # Handle general file I/O or other unexpected errors
...
def user_exists(username):
    ...
    try:
        ...
        # If loading fails (e.g., file corruption), assume user does not exist
        return False
